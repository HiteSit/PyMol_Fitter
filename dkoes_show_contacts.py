...
def show_sele_protein_contacts():
    ...
    # Generate a result name based on the object(s) in 'sele'
    if objects:
        # Join all object names with underscores
        result_name = "_".join(objects) + "_contacts"
    else:
        # Fallback name if no objects are found in 'sele'
        result_name = "protein_contacts"
    
    # Execute the contacts command with the generated name
    show_contacts(selection, protein, result=result_name)

# Bind the function to the F3 key
cmd.set_key('F3', show_sele_protein_contacts)
